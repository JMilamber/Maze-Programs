...
def split(area_To_Split, hall_S, hall_Area_dict, id):
    ...
    if h_V == 0:
        ...
        id = id + 1
        hall_Area_dict["0" + str(id)] = Area(x, y + wall_num * hall_S, width,
                                             height - wall_num * hall_S)
        ...
        id = id + 1
        hall_Area_dict["0" + str(id)] = Area(x + wall_num * hall_S, y,
                                             width - wall_num * hall_S, height)
...
def draw(maze_S, hall_S, p):
    ...
    rect = Rectangle(Point(offset, offset),
                     Point(maze_S + offset, maze_S + offset))
    ...
    rect.setWidth(2)
    rect.draw(win)
    message = Text(Point(maze_S/2 + offset,
                         maze_S + (offset * 2) - (offset/2)),
                   "Calculating...")
    message.draw(win)
    # hall_S = 25
    id = 0
    ...
    first_Line = Line(Point(offset,
                            offset + (hall_S * maze_Start) + hall_S - 1),
                      Point(offset, offset + (hall_S * maze_Start) + 1))
    first_Line.setFill(color_rgb(240, 240, 240))
    first_Line.setWidth(2)
    first_Line.draw(win)
    start_Msg = Text(Point(offset/2,
                           offset + (hall_S * maze_Start) + (hall_S/2)),
                     "Start->")
    start_Msg.draw(win)

    first_Line = Line(Point(offset + maze_S, offset + (hall_S * maze_End) + 1),
                      Point(offset + maze_S,
                            offset + (hall_S * maze_End) + hall_S - 1))
    first_Line.setFill(color_rgb(240, 240, 240))
    first_Line.setWidth(2)
    first_Line.draw(win)
    end_Msg = Text(Point(offset * 1.5 + maze_S,
                         offset + (hall_S * maze_End) + (hall_S/2)), "<-End")
    end_Msg.draw(win)
    ...
    while done == 0:
        ...
        if done == 0:
            ...
            hall_Area_dict, h_V, wall_num, id = split(area_To_Split, hall_S,
                                                      hall_Area_dict, id)
            ...
            if h_V == 0:
                ...
                walls_list.append(Wall(area_To_Split.get_X(),
                                       area_To_Split.get_Y() +
                                       (wall_num * hall_S),
                                       area_To_Split.get_X() +
                                       area_To_Split.get_Width(),
                                       area_To_Split.get_Y() +
                                       (wall_num * hall_S)))
                # add the new wall to the wall list, which the program uses to
                # draw the walls after creating all of them.
                wall_To_Remove = (random.randint(1, area_To_Split.get_Width() /
                                                 hall_S) - 1) * hall_S
                # print("H Wall to remove: ", wall_To_Remove/hall_S)
                passage_list.append(Wall(area_To_Split.get_X() +
                                         wall_To_Remove + 1,
                                         area_To_Split.get_Y() +
                                         (wall_num * hall_S),
                                         area_To_Split.get_X() +
                                         wall_To_Remove + hall_S - 1,
                                         area_To_Split.get_Y() +
                                         (wall_num * hall_S)))
                ...
                walls_list.append(Wall(area_To_Split.get_X() +
                                       ...
                                       (wall_num * hall_S),
                                       area_To_Split.get_Y() +
                                       area_To_Split.get_Height()))
                # add the new wall to the wall list, which the program uses to
                # draw the walls after creating all of them.
                wall_To_Remove = (random.randint(1,
                                                 area_To_Split.get_Height() /
                                                 hall_S) - 1) * hall_S
    ...
    message.undraw()
    message = Text(Point(maze_S/2 + offset, maze_S + (offset * 2) -
                         (offset/2)),
                   "Drawing...")
    message.draw(win)
    ...
    for f in range(0, len(walls_list)):
        ...
        if hall_S == 10 and maze_S > 150:
            ...
            time.sleep(.3)
        wall = Line(Point(W.get_X() + offset, W.get_Y() + offset),
        ...
        P = passage_list[f]
        passage = Line(Point(P.get_X() + offset, P.get_Y() + offset),
...
def main():
    ...
    main_Label = tkinter.Label(main,
                               text="Recursive Division").grid(row=0, column=1,
                                                               columnspan=1)
    ...
    filler_2 = tkinter.Label(main, text="                (100 - 800)        " +
                             "        |     ").grid(row=2, column=2)
    filler_3 = tkinter.Label(main, text="    (small, medium, or large)  |   " +
                             "  ").grid(row=3, column=2)
    filler_4 = tkinter.Label(main,
                             text="-----------------------------------------" +
                             "------------------------|").grid(row=4, column=0,
                                                               columnspan=3)
